...
public class Graficacion extends Canvas {
	...
	String Titulo = "DIAGRAMA BHPBA vs QD", Variable1 = "BHPBA",
			Variable2 = "QD";
	...
	void Datos(double x[], double y[], double width, double height, String tit,
	...
	public void GraficarTodo(Graphics g) {
		...
		g.drawString(Variable2, (this.getSize().width / 2) - 40,
				this.getSize().height - 10);
		g.drawString(Titulo, (this.getSize().width / 2) - 80, 10);
		for (i = 0; i < numd; i++) {
			etiqx[i] = (((int) (etiqx[i] + 0.5)));
			etiqy[i] = (((int) (etiqy[i] + 0.5)));
			g.drawString(Integer.toString((int) (etiqx[i])), 50 + posx[i] - 2,
					pos_ejey + 20);
			g.drawLine(50 + posx[i], pos_ejey - 5, 50 + posx[i], pos_ejey + 5);

			g.drawString(Integer.toString((int) etiqy[i]), pos_ejex - 50, MaxY
					- posy[i]);
			g.drawLine(pos_ejex - 5, MaxY - posy[i], pos_ejex + 5, MaxY
					- posy[i]);
		}
		...
		for (j = 0; j <= mm; j++) {
			...
			for (i = 0; i < n - 1; i++)
				g.drawLine(50 + x[j][i], MaxY - y[j][i], 50 + x[j][i + 1], MaxY
						- y[j][i + 1]);
		}
		// /////////////////////////////////////////
